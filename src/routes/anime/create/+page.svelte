<script lang="ts">
    ...
    interface SearchResults {
        [siteId: string]: AnimeResult[];
    }

    interface SelectedData {
        [siteId: string]: AnimeResult | null;
    }
    ...
    // Define site configurations for anime
    const sites: SiteConfig[] = [
        {
            ...
            helpText: [
                "Search for the anime you want to add",
                "Copy the numeric ID from the URL of the page",
                "Example: https://myanimelist.net/anime/<span style=\"color: red; font-weight: bold\">16498</span>/Shingeki_no_Kyojin"
            ]
        },
        {
            id: "sc",
            ...
            helpText: [
                ...
                "Example: https://www.senscritique.com/serie/<span style=\"color: red; font-weight: bold\">attack-on-titan/12345</span>/"
            ]
        }
    ];
    ...
    const findAnimeInfo = async (): Promise<void> => {
    ...
    const createAnime = async (e: SubmitEvent): Promise<void> => {
        e.preventDefault();
        const createButton = document.getElementById("createButton");
        if (createButton) createButton.setAttribute("disabled", "");
        
        creationMessage = {
            message: "Creating anime...",
            color: "blue"
        };
        ...
        try {
            ...
            if (!response.ok) {
                creationMessage = {
                    message: json.message || 'Failed to create anime',
                    color: "red"
                };
                return;
            }
            ...
            creationMessage = {
                message: "Anime created successfully! Redirecting...",
                color: "green"
            };
            
            setTimeout(() => {
                window.location.href = `/anime/${json.id}`;
            }, 1000);
        } catch (error) {
...
<div class="max-w-4xl mx-auto p-4">
    <h2 class="text-4xl font-extrabold dark:text-white mb-6">Create Anime</h2>
    ...
    <div class="mb-8">
        <div class="flex gap-2">
            <Input
                ...
                {/snippet}
                {#snippet right()}
                    <Button size="xs" onclick={findAnimeInfo}>Search</Button>
                {/snippet}
            </Input>
        </div>
    ...
    <form on:submit={createAnime} class="space-y-6">
        ...
        <div class="pt-4">
            <div class="flex flex-col sm:flex-row items-center gap-4">
                <Button id="createButton" type="submit" size="lg">Create Anime</Button>
                {#if creationMessage}
