<script lang="ts">
    ...
    interface SearchResults {
        [siteId: string]: TvShowResult[];
    }

    interface SelectedData {
        [siteId: string]: TvShowResult | null;
    }
    ...
    const sites: SiteConfig[] = [
        {
            ...
            helpText: [
                ...
                "Example: https://www.themoviedb.org/tv/<span style=\"color: red; font-weight: bold\">823464</span>-godzilla-x-kong-the-new-empire/"
            ]
        },
        {
            id: "sc",
            ...
            helpText: [
                ...
                "Example: https://www.senscritique.com/serie/<span style=\"color: red; font-weight: bold\">her/1301677</span>/"
            ]
        },
        {
            id: "mc",
            ...
            helpText: [
                ...
                "Example: https://www.metacritic.com/tv/<span style=\"color: red; font-weight: bold\">the-truman-show</span>/"
            ]
        },
        {
            id: "rt", 
            ...
            helpText: [
                "Search for the tv show you want to add",
                "Copy the text ID from the URL of the page",
                "Example: https://www.rottentomatoes.com/tv/<span style=\"color: red; font-weight: bold\">lost_in_translation</span>/"
            ]
        }
    ];
    ...
    const findTvshowInfo = async (): Promise<void> => {
        if (!searchTerm.trim()) return;
        
        searchStatus = 'started';
        try {
            const response = await fetch(`/api/tvshow/findTvshowInfo?query=${encodeURIComponent(searchTerm)}`);
            if (!response.ok) {
    ...
    const createTvshow = async (e: SubmitEvent): Promise<void> => {
        e.preventDefault();
        const createButton = document.getElementById("createButton");
        if (createButton) createButton.setAttribute("disabled", "");
        
        creationMessage = {
            message: "Creating TV Show...",
            color: "blue"
        };
        ...
        try {
            const response = await fetch("/api/tvshow/create", {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                creationMessage = {
                    message: json.message || 'Failed to create TV Show',
                    color: "red"
                };
                return;
            }
            ...
            creationMessage = {
                message: "TV Show created successfully! Redirecting...",
                color: "green"
            };
            
            setTimeout(() => {
                window.location.href = `/tvshow/${json.id}`;
            }, 1000);
        } catch (error) {
...
<div class="max-w-4xl mx-auto p-4">
    <h2 class="text-4xl font-extrabold dark:text-white mb-6">Create TV Show</h2>
    ...
    <form on:submit={createTvshow} class="space-y-6">
        ...
        <div class="pt-4">
            <div class="flex flex-col sm:flex-row items-center gap-4">
                <Button id="createButton" type="submit" size="lg">Create TV Show</Button>
                {#if creationMessage}
